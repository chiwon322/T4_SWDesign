...
import com.team4.finding_sw_developers.Notification.Client;
...
import com.team4.finding_sw_developers.Notification.Data;
...
public class MessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* toolbar.setNavigationOnClickListener(new View.OnClickListener() {
        ...
        recyclerView = findViewById(R.id.third_recycler_view);
    ...
    private void sendMessage(String sender, final String receiver, String message) {
        ...
        chatRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!snapshot.exists()) {         //존재하지 않으면
                    HashMap<String, Object> inHashMap = new HashMap<>();
                    inHashMap.put("id", userid);
                    inHashMap.put("timestamp", ServerValue.TIMESTAMP);
                    isitNew = true;
                    chatRef.setValue(inHashMap);
                    doPairUpdate(isitNew);
                } else {
                    HashMap<String, Object> inHashMap = new HashMap<>();
                    inHashMap.put("timestamp", ServerValue.TIMESTAMP);
                    chatRef.updateChildren(inHashMap);
                    doPairUpdate(isitNew);
                }
            }
    ...
    private void doPairUpdate(boolean isitNew) {
        ...
        if (isitNew) {
            //chatRefReceiver.child("id").setValue(fuser.getUid());
            HashMap<String, Object> inHashMap = new HashMap<>();
            inHashMap.put("id", fuser.getUid());
            inHashMap.put("timestamp", ServerValue.TIMESTAMP);
            chatRefReceiver.setValue(inHashMap);
        } else {
            HashMap<String, Object> inHashMap = new HashMap<>();
            inHashMap.put("timestamp", ServerValue.TIMESTAMP);
            chatRefReceiver.updateChildren(inHashMap);
        }
    }
